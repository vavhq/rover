...
import { isBlacklisted } from "@/core/blocklist";
import { config } from "@/core/config";
import { getBlockedDevs, isDevBlocked } from "@/core/dev-blocklist";
...
async function fetchDiscordSignalCandidates() {
  const res = await fetch(`${getRelayApiBase()}/signals/discord/candidates`, {
...
export async function discoverPools({ page_size = 50 } = {}) {
  ...
  const url = useServerDiscovery
    ? `${getRelayApiBase()}/discovery/pools?` +
      `page_size=${page_size}` +
  ...
  const res = await fetch(url, {
    headers: useServerDiscovery ? getRelayApiHeaders() : {},
  });
...
export async function getPoolDetail({ pool_address, timeframe = "5m" }) {
  const useServerDiscovery = !!config.api.publicApiKey;
  const url = useServerDiscovery
    ? `${getRelayApiBase()}/discovery/pools/${pool_address}?timeframe=${encodeURIComponent(timeframe)}`
    : `${POOL_DISCOVERY_BASE}/pools?` +
      `page_size=1` +
