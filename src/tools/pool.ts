...
import { computeDeployAmount, config } from "@/core/config";
import { appendDecision } from "@/core/decision-log";
import { recordPerformance } from "@/core/memory";
import { isBaseMintOnCooldown, isPoolOnCooldown } from "@/core/poollog";
import {
  getTrackedPosition,
  markInRange,
  markOutOfRange,
...
import { getRelayApiHeaders, getRoverRequestAgentId, relayApiJson } from "@/integrations/relay-api";
import { log } from "@/platform/logger";
import { normalizeMint } from "@/tools/treasury";
...
function shouldUseExecutionRelay() {
  return !!config.api.relayEnabled;
}

function shouldUseExecutionRelayForDeploy() {
...
const METEORA_INIT_BIN_ARRAY_DISCRIMINATOR = Buffer.from([
  35, 86, 19, 185, 78, 212, 75, 211,
]).toString("hex");
const METEORA_INIT_BITMAP_EXTENSION_DISCRIMINATOR = Buffer.from([
...
async function assertRangeDoesNotRequireBinArrayInitialization(pool, minBinId, maxBinId) {
  ...
  if (missing.length > 0) {
    ...
    const sample = missing
      .slice(0, 3)
      .map((entry) => `${entry.index}:${entry.address.slice(0, 8)}`)
      .join(", ");
    throw new Error(
...
function getDlmmInstructionDiscriminators(serialized) {
  ...
  try {
    ...
    return legacy.instructions
      .map((ix) =>
        ix.programId.toString() === dlmmProgramId
          ? Buffer.from(ix.data || [])
              .subarray(0, 8)
              .toString("hex")
          : null
...
export async function deployPosition({
  ...
  if (process.env.DRY_RUN === "true") {
    ...
    return {
      ...
      message: "DRY RUN is enabled — this is a simulation, no transaction was sent.",
    };
  }
  ...
  if (shouldUseExecutionRelayForDeploy()) {
    try {
      const wallet = getWallet();
      log(
        "deploy",
        `Execution relay deploy: ${pool_address} activeBin ${activeBin.binId} bins ${minBinId}->${maxBinId} amountY=${finalAmountY}`
      );
      const order = await relayApiJson("/execution/zap-in/order", {
        method: "POST",
        headers: getRelayApiHeaders({ json: true }),
        body: JSON.stringify({
          agentId: getRoverRequestAgentId(),
      ...
      if (addLiquidityUnsigned.length + swapUnsigned.length === 0) {
        throw new Error(
          "EXECUTION_RELAY_EMPTY: relay returned no transactions for deploy request."
        );
      }
      ...
      const submit = await relayApiJson("/execution/zap-in/submit", {
        method: "POST",
        headers: getRelayApiHeaders({ json: true }),
        body: JSON.stringify({
      ...
      const matching =
        refreshed?.positions?.find(
          (position) =>
            ...
            position.lower_bin === minBinId &&
            position.upper_bin === maxBinId
...
async function fetchExternalOpenPositions() {
  // Public Rover build: no external proprietary dependencies.
  return {};
}
...
export async function getPositionPnl({ pool_address, position_address }) {
  ...
  if (shouldUseExecutionRelay()) {
    try {
      const payload = await fetchOpenPositionsFromRelay({
        walletAddress,
        agentId: getRoverRequestAgentId(),
      });
      ...
      log("pnl_warn", "Relay did not include requested position; falling back to Meteora PnL path");
    } catch (error) {
      log(
        "pnl_warn",
        `Relay PnL lookup failed; falling back to Meteora PnL path: ${error.message}`
      );
    }
  }
  try {
    ...
    const unclaimedUsd =
      parseFloat(p.unrealizedPnl?.unclaimedFeeTokenX?.usd || 0) +
      parseFloat(p.unrealizedPnl?.unclaimedFeeTokenY?.usd || 0);
...
function deriveExternalPnlPct(lpData, solMode = false) {
...
async function fetchOpenPositionsFromRelay({ walletAddress, agentId }) {
  const search = new URLSearchParams({
    owner: walletAddress,
    agentId: agentId || "agent-local",
  });
  const payload = await relayApiJson(`/positions/open?${search.toString()}`, {
...
export async function getMyPositions({ force = false, silent = false } = {}) {
  ...
  _positionsInflight = (async () => {
    try {
      if (shouldUseExecutionRelay()) {
        try {
          if (!silent) log("positions", "Fetching open positions via execution relay...");
          const result = await fetchOpenPositionsFromRelay({
            walletAddress,
            agentId: getRoverRequestAgentId(),
          });
          ...
          log(
            "positions_warn",
            `Execution relay failed; falling back to Meteora/local positions path: ${error.message}`
          );
        }
      }

      // Portfolio API discovers open pools/positions for this wallet.
      // Detailed range data stays on Meteora PnL API; value/PnL can be optionally overridden by external enrichment below.
      ...
      const pnlMaps = await Promise.all(
        pools.map((pool) => fetchDlmmPnlForPool(pool.poolAddress, walletAddress))
      );
      pools.forEach((pool, i) => {
        binDataByPool[pool.poolAddress] = pnlMaps[i];
      });
      const externalByPosition = await fetchExternalOpenPositions(walletAddress);
      ...
      for (const pool of pools) {
        for (const positionAddress of pool.listPositions || []) {
          ...
          const lpData = externalByPosition[positionAddress] || null;
          ...
          const reportedPnlPct = lpData
            ? parseFloat(
                config.management.solMode
                  ? lpData.pnl?.percentNative || 0
                  : lpData.pnl?.percent || 0
              )
            : binData
              ? parseFloat(
                  config.management.solMode
          ...
          const derivedPnlPct = lpData
            ? deriveExternalPnlPct(lpData, config.management.solMode)
            : binData
          ...
          positions.push({
            ...
            unclaimed_fees_usd: lpData
              ? Math.round(
                  (config.management.solMode
                    ? safeNum(lpData.unCollectedFeeNative)
                    : safeNum(lpData.unCollectedFee)) * 10000
                ) / 10000
              : binData
                ? Math.round(
                    (config.management.solMode
                      ...
                      : parseFloat(binData.unrealizedPnl?.unclaimedFeeTokenX?.usd || 0) +
                        parseFloat(binData.unrealizedPnl?.unclaimedFeeTokenY?.usd || 0)) * 10000
                  ) / 10000
                : null,
            total_value_usd: lpData
              ? Math.round(
                  (config.management.solMode
                    ...
                    : safeNum(lpData.value)) * 10000
                ) / 10000
              : binData
                ? Math.round(
                    (config.management.solMode
                      ? parseFloat(binData.unrealizedPnl?.balancesSol || 0)
                      : parseFloat(binData.unrealizedPnl?.balances || 0)) * 10000
                  ) / 10000
                : null,
            ...
            collected_fees_usd: lpData
              ? Math.round(
                  (config.management.solMode
                    ...
                    : safeNum(lpData.collectedFee)) * 10000
                ) / 10000
              : binData
                ? Math.round(
                    parseFloat(
            ...
            pnl_usd: lpData
              ? Math.round(
                  (config.management.solMode
                    ...
                    : safeNum(lpData.pnl?.value)) * 10000
                ) / 10000
              : binData
                ? Math.round(
                    parseFloat(
            ...
            pnl_true_usd: lpData
              ...
              : binData
                ...
                : null,
            pnl_pct: lpData || binData ? Math.round(reportedPnlPct * 100) / 100 : null,
  ...
  try {
    ...
    const positions = raw.map((r) => {
      ...
      return {
        ...
        total_value_usd:
          Math.round((p ? parseFloat(p.unrealizedPnl?.balances || 0) : 0) * 100) / 100,
        pnl_usd: Math.round((p?.pnlUsd ?? 0) * 100) / 100,
        pnl_pct: Math.round((p?.pnlPctChange ?? 0) * 100) / 100,
        age_minutes: p?.createdAt ? Math.floor((Date.now() - p.createdAt * 1000) / 60000) : null,
      };
...
export async function claimFees({ position_address }) {
  ...
  if (process.env.DRY_RUN === "true") {
    return {
      ...
      message: "DRY RUN is enabled — no transaction was sent.",
    };
  }
...
export async function closePosition({ position_address, reason }) {
  ...
  try {
    ...
    if (shouldUseExecutionRelay()) {
      ...
      const quotes = await relayApiJson("/execution/zap-out/quotes", {
        method: "POST",
        headers: getRelayApiHeaders({ json: true }),
        body: JSON.stringify({
          agentId: getRoverRequestAgentId(),
      ...
      const order = await relayApiJson("/execution/zap-out/order", {
        method: "POST",
        headers: getRelayApiHeaders({ json: true }),
        body: JSON.stringify({
          agentId: getRoverRequestAgentId(),
      ...
      if (closeUnsigned.length + swapUnsigned.length === 0) {
        throw new Error("EXECUTION_RELAY_EMPTY: relay returned no transactions for close request.");
      }

      const submit = await relayApiJson("/execution/zap-out/submit", {
        method: "POST",
        headers: getRelayApiHeaders({ json: true }),
        body: JSON.stringify({
          requestId: order.requestId,
      ...
      for (let attempt = 0; attempt < 4; attempt++) {
        try {
          ...
          log(
            "close_warn",
            `Relay close still appears open after submit (attempt ${attempt + 1}/4)`
          );
        } catch (e) {
          log(
            "close_warn",
            `Relay close verification failed (attempt ${attempt + 1}/4): ${e.message}`
          );
        }
          ...
          for (let attempt = 0; attempt < 6; attempt++) {
            ...
            if (res.ok) {
              ...
              const posEntry = (data.positions || []).find(
                (entry) => entry.positionAddress === position_address
              );
              if (posEntry) {
      ...
      try {
        ...
        for (let attempt = 0; attempt < 6; attempt++) {
          ...
          if (res.ok) {
            const data = await res.json();
            const posEntry = (data.positions || []).find(
              (p) => p.positionAddress === position_address
            );
            if (posEntry) {
              ...
              if (shouldRejectClosedPnl(nextPnlPct, reason || tracked?.close_reason)) {
                log(
                  "close_warn",
                  `Rejected unsettled closed PnL for ${position_address.slice(0, 8)} on attempt ${attempt + 1}/6: ${nextPnlPct.toFixed(2)}%`
                );
              } else {
                pnlUsd = nextPnlUsd;
                pnlPct = nextPnlPct;
              ...
              log(
                "close_warn",
                `Position not found in status=closed response (attempt ${attempt + 1}/6) — may still be settling`
              );
            }
          }
