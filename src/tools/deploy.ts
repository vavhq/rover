...
import { blockDev, listBlockedDevs, unblockDev } from "@/core/dev-blocklist";
import {
  addLesson,
  clearAllLessons,
  clearPerformance,
...
import { addPoolNote, getPoolMemory } from "@/core/poollog";
import { setPositionInstruction } from "@/core/registry";
import {
  addSmartWallet,
...
import { log, logAction } from "@/platform/logger";
import { notifyClose, notifyDeploy, notifySwap } from "@/platform/notify";
import { getTokenHolders, getTokenInfo, getTokenNarrative } from "@/tools/asset";
import {
  claimFees,
  closePosition,
  deployPosition,
...
import { studyTopLPers } from "@/tools/radar";
import { discoverPools, getPoolDetail, getTopCandidates } from "@/tools/scan";
...
const toolMap = {
  ...
  update_config: ({ changes, reason = "" }) => {
    ...
    const CONFIG_MAP = {
      ...
      // swarm
      swarmUrl: ["swarm", "url"],
      scoutKey: ["swarm", "scoutKey"],
      agentId: ["swarm", "agentId"],
      // discovery / relay API
      publicApiKey: ["api", "publicApiKey"],
      discoveryApiUrl: ["api", "url"],
      relayApiBaseUrl: ["api", "url"],
      relayEnabled: ["api", "relayEnabled"],
      // chart indicators
    ...
    if (success) {
      if (name === "swap_token" && result.tx) {
        notifySwap({
          ...
          outputSymbol:
            args.output_mint === "So11111111111111111111111111111111111111112" ||
        ...
        if (args.reason?.toLowerCase().includes("yield")) {
