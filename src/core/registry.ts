/**
 * Persistent agent state — stored in registry.json.
 *
 * Tracks Stake metadata that isn't available on-chain:
 * - When a Stake was entered
...
function load() {
  ...
  try {
    ...
    log("state_error", `Failed to read registry.json: ${err.message}`);
