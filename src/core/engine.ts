...
import { buildSystemPrompt } from "@/core/cortex";
import { tools } from "@/tools/definitions";
import { executeTool } from "@/tools/deploy";
...
const CONFIG_READ_ONLY_INTENTS =
  /\b(check|show|what(?:'s| is)?|review|inspect|see)\b.*\b(config|settings?|thresholds?)\b/i;
...
export async function agentLoop(
  ...
  if (agentType === "SCREENER") {
    try {
      const { getWeightsSummary } = await import("@/core/signal-weights");
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    try {
      ...
      if (!response.choices?.length) {
        ...
        throw new Error(
          `API returned no choices: ${response.error?.message || JSON.stringify(response)}`
        );
      }
