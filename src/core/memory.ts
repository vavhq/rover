...
const LESSONS_FILE = workspacePath("memory.json");
...
export async function recordPerformance(perf) {
  ...
  save(data);
  if (lesson) {
    void pushSharedLesson(lesson);
  }

  // Update pool-level memory
  if (perf.pool) {
    const { recordPoolDeploy } = await import("@/core/poollog");
  ...
  if (data.performance.length % MIN_EVOLVE_POSITIONS === 0) {
    const { config, reloadScreeningThresholds } = await import("@/core/config");
    ...
    if (config.darwin?.enabled) {
      const { recalculateWeights } = await import("@/core/signal-weights");
...
export function addLesson(rule, tags = [], { pinned = false, role = null } = {}) {
  ...
  const lesson = {
    ...
    outcome: "manual",
    sourceType:
      tags.includes("self_tune") || tags.includes("config_change") ? "config_change" : "manual",
    pinned: !!pinned,
    role: role || null,
    created_at: new Date().toISOString(),
  };
