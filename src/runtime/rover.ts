...
import { computeDeployAmount, config, reloadScreeningThresholds } from "@/core/config";
import { appendDecision } from "@/core/decision-log";
import { agentLoop } from "@/core/engine";
import { evolveThresholds, getPerformanceSummary } from "@/core/memory";
...
import { ensureAgentId, isSwarmEnabled } from "@/core/swarm";
import { checkSmartWalletsOnPool } from "@/core/tracker";
...
import { getTokenInfo, getTokenNarrative } from "@/tools/asset";
import { executeTool, registerCronRestarter } from "@/tools/deploy";
...
function buildPrompt() {
  ...
  const scrn = formatCountdown(
    nextRunIn(timers.screeningLastRun, config.schedule.screeningIntervalMin)
  );
...
export async function runScreeningCycle({ silent = false } = {}) {
  ...
  try {
    ...
    const _weightsSummary = config.darwin?.enabled ? getWeightsSummary() : null;
...
export function startCronJobs() {
  // Multi-instance guard: only one Rover per workspace directory.
  acquireInstanceLock();

  stopCronJobs(); // stop any running tasks before (re)starting

  // Crash recovery behavior: on startup, immediately run one management pass
  // so open Stakes are evaluated even if the next cron tick is far away.
  if (!_managementBusy) {
    _managementBusy = true;
    runManagementCycle({ silent: true })
      .catch((e) => log("cron_error", `Startup management failed: ${e?.message || String(e)}`))
      .finally(() => {
        _managementBusy = false;
      });
  }
...
function formatCandidates(candidates) {
  if (!candidates.length) return "  No pools passed the current filters.";
...
function getDeterministicCloseRule(position, managementConfig) {
  ...
  const pnlSuspect = (() => {
    ...
    if (tracked?.amount_sol && (position.total_value_usd ?? 0) > 0.01) {
      log(
        "cron_warn",
        `Suspect PnL for ${position.pair}: ${position.pnl_pct}% but position still has value — skipping PnL rules`
      );
      return true;
    }
    return false;
...
function describeLatestCandidates(limit = 5) {
  ...
  const age = _latestCandidatesAt
    ? new Date(_latestCandidatesAt).toLocaleString("en-US", { hour12: false })
    : "unknown";
...
function formatWalletStatus(wallet, positions) {
  ...
  const hive = isSwarmEnabled() ? "on" : "off";
  return [
...
function formatConfigSnapshot() {
  return [
    ...
    `Swarm: ${isSwarmEnabled() ? "enabled" : "disabled"}${config.swarm?.agentId ? ` | ${config.swarm.agentId}` : ""}`,
  ].join("\n");
}
...
function settingValue(key) {
  const values = {
    ...
    relayEnabled: config.api.relayEnabled,
...
function renderSettingsMenu(page = "main") {
  ...
  const summary = [
    title,
    "",
    `Mode: ${config.management.solMode ? "SOL" : "USD"} | Relay: ${config.api.relayEnabled ? "on" : "off"}`,
  ...
  const footer = [
    [settingButton("Refresh", `cfg:page:${page}`), settingButton("Close", "cfg:close")],
  ];
  ...
  if (page === "risk") {
    ...
    rows = [
      [
        ...
        toggleButton("requireAllIntervals", "Require all TF"),
      ],
      [
    ...
    rows = [
      [toggleButton("solMode", "SOL mode"), toggleButton("relayEnabled", "Relay")],
      [
        toggleButton("chartIndicatorsEnabled", "Chart indicators"),
        toggleButton("trailingTakeProfit", "Trailing TP"),
      ],
...
async function applySettingsMenuCallback(msg) {
  ...
  if (action === "show") {
    await answerCallbackQuery(msg.callbackQueryId);
    await editMessageWithButtons(formatConfigSnapshot(), msg.messageId, [
  ...
  if (action === "toggle") {
    ...
    if (["deployAmountSol", "gasReserve", "maxDeployAmount"].includes(key))
...
function formatHelpText() {
  return [
    ...
    "/swarm — Swarm sync status",
    "/swarm pull — fetch thresholds now",
...
async function deployLatestCandidate(index) {
  ...
  const binsBelow = Math.max(
    35,
    Math.min(90, Math.round(35 + ((Number(candidate.volatility) || 0) / 5) * 55))
  );
...
async function telegramHandler(msg) {
  ...
  if (text === "/settings" || text === "/menu" || text === "/configmenu") {
    await showSettingsMenu().catch((e) =>
      sendMessage(`Settings error: ${e.message}`).catch(() => {})
    );
    return;
  }
    ...
    try {
      ...
      for (const pos of positions) {
        try {
          const result = await closePosition({ position_address: pos.position });
          results.push(
            `${pos.pair}: ${result.success ? "closed" : `failed (${result.error || "unknown"})`}`
          );
        } catch (error) {
  ...
  if (text === "/swarm" || text === "/swarm pull") {
    try {
      ...
      const isManualPull = text === "/swarm pull";
      const { fetchThresholds } = await import("@/core/swarm");
      const thresholds = isManualPull ? await fetchThresholds() : null;
      await sendMessage(
        [
          "Swarm: enabled",
          `Agent ID: ${agentId}`,
          `URL: ${config.swarm?.url || "unknown"}`,
          isManualPull ? `Thresholds: ${thresholds?.ok ? "ok" : "failed"}` : null,
        ]
          .filter(Boolean)
          .join("\n")
      ).catch(() => {});
    } catch (e) {
      await sendMessage(`Swarm error: ${e.message}`).catch(() => {});
    }
    return;
  }
      ...
      await runBusy(async () => {
        ...
        if (poolArg) {
          ...
          if (!candidates.length) {
            console.log("No pools available to study right now.\n");
            return;
          }
...
Focus on: hold duration, entry/exit timing, what win rates look like, whether scalpers or holders dominate.`,
    ...
    if (input === "/evolve") {
      await runBusy(async () => {
        ...
        const lessonsData = JSON.parse(fs.default.readFileSync("memory.json", "utf8"));
        ...
        if (!result || Object.keys(result.changes).length === 0) {
          ...
          console.log("\nSaved to local config. Applied immediately.\n");
        }
      });
      return;
    }
