/**
 * Rover Discord signal listener (selfbot).
 * Watches configured channels for Solana addresses and runs the pre-check pipeline.
 *
 * Env (repository root `.env`):
 *   DISCORD_USER_TOKEN, DISCORD_GUILD_ID, DISCORD_CHANNEL_IDS, DISCORD_MIN_FEES_SOL
 */
// @ts-nocheck
...
if (!TOKEN) {
  console.error("CONFIG ERROR: DISCORD_USER_TOKEN is missing in the repo root .env");
  process.exit(1);
}
if (!GUILD_ID) {
  console.error("CONFIG ERROR: DISCORD_GUILD_ID is missing in the repo root .env");
  process.exit(1);
}
if (CHANNEL_IDS.length === 0) {
  console.error(
    "CONFIG ERROR: DISCORD_CHANNEL_IDS is missing in the repo root .env (comma-separated channel IDs)"
  );
  process.exit(1);
}
...
client.on("ready", () => {
  console.log(`\n[rover signal] Connected as ${client.user?.tag}`);
  const guild = client.guilds.cache.get(GUILD_ID);
  if (!guild) {
    console.warn(`CONFIG WARN: Guild ${GUILD_ID} was not found in cache. Check DISCORD_GUILD_ID.`);
  } else {
